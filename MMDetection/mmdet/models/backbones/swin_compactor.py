...
class PHMLinear(nn.Module):
    ...
    def init_W(self):
        if self.w_init == "glorot-normal":
            raise ValueError("Glorot Normal is a Wrong Key.")
            # if self.factorized_phm:
    ...
    def forward(
        ...
        H = kronecker_product_einsum_batched(
            self.phm_rule, W if self.factorized_phm else self.W
        ).sum(0)
...
# copy from kadaptation.
class HyperComplexAdapter(BaseModule):
    ...
    def __init__(
        ...
        in_channels,
        inner_dim=None,
        init_compactor_weights=True,
        add_layer_norm_before=True,
        add_layer_norm_after=False,
        residual_before_ln=True,
    ):
        super().__init__()
        self.activation = Activations("gelu".lower())
        ...
        if self.add_layer_norm_before:
            self.adapter_norm_before = nn.LayerNorm(self.in_channels)
            # seq_list.append(self.adapter_norm_before)
        self.inner_dim = inner_dim
        if inner_dim is None:
            self.inner_dim = self.in_channels // 2
        ...
        if self.add_layer_norm_after:
            self.adapter_norm_after = nn.LayerNorm(self.in_channels)

        if init_compactor_weights:
            self.adapter_down.apply(self.init_compactor_weights)
            self.adapter_up.apply(self.init_compactor_weights)
...
class SwinBlock(BaseModule):
    ...
    def __init__(
        ...
        self.compacter = HyperComplexAdapter(embed_dims, inner_dim=INNER_DIM,)
...
class SwinCompactor(BaseModule):
    """ Swin Transformer
    ...
    def __init__(
        ...
        super(SwinCompactor, self).__init__(init_cfg=init_cfg)
        ...
        #     if isinstance(sub_m, PHMLinear):
        #         print(f"set phm rule for {n}.")
        #         sub_m.set_phm_rule(phm_rule=self.phm_rule)
    ...
    def after_forward(self):
        for n, sub_m in self.stages.named_modules():
            if isinstance(sub_m, PHMLinear):
                # print(f"after forward for {n}.")
                sub_m.unset_phm_rule()

    def train(self, mode=True):
        """Convert the model into training mode while keep layers freezed."""
        super(SwinCompactor, self).train(mode)
        self._freeze_stages()
