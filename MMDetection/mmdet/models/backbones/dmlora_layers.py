...
class Linear(nn.Linear, LoRALayer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if r > 0:
            ...
            self.lora_As = nn.ParameterList(lora_As)
            lora_Bs = [
                nn.Parameter(self.weight.new_zeros((out_features, r)))
                for _ in range(nr_experts)
            ]
            self.lora_Bs = nn.ParameterList(lora_Bs)

            # self.scaling = self.lora_alpha / self.r
            self.scaling = nn.Parameter((self.lora_alpha / self.r) * torch.ones((1,)))
            # Freezing the pre-trained weight matrix
            self.weight.requires_grad = False
        self.reset_parameters()
        if fan_in_fan_out:
    ...
    def train(self, mode: bool = True):
        ...
        if mode:
            if self.merge_weights and self.merged:
                # Make sure that the weights are not merged
                lora_A = sumParamList(self.lora_As) / len(self.lora_As)
                lora_B = sumParamList(self.lora_Bs) / len(self.lora_Bs)
                if self.r > 0:
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.r > 0 and not self.merged:
            ...
            random_iA = random.randint(0, len(self.lora_As) - 1)
            lora_A = self.lora_As[random_iA]
            random_iB = random.randint(0, len(self.lora_Bs) - 1)
            lora_B = self.lora_Bs[random_iB]
            if self.r > 0:
    ...
    def __init__(
        ...
        **kwargs
        ...
        if r > 0 and any(enable_lora):
            ...
            lora_As = [nn.Parameter(
                self.weight.new_zeros((r * sum(enable_lora), in_features))
            ) for _ in range(nr_experts)]
            self.lora_As = nn.ParameterList(lora_As)

            lora_Bs = [nn.Parameter(
                self.weight.new_zeros(
                    (out_features // len(enable_lora) * sum(enable_lora), r)
                )
            ) for _ in range(nr_experts)]
        ...
        if mode:
            if self.merge_weights and self.merged:
                ...
                if self.r > 0 and any(self.enable_lora):
                    ...
                    # ).squeeze(0)
                    delta_w = F.conv1d(
                        lora_A.data.unsqueeze(0),
                        lora_B.data.unsqueeze(-1),
                        groups=sum(self.enable_lora),
                    ).squeeze(0)
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.merged:
            ...
            if self.r > 0:
                ...
                #     groups=sum(self.enable_lora),
                # ).transpose(-2, -1)
                after_A = F.linear(self.lora_dropout(x), lora_A)
                after_B = F.conv1d(
                    after_A.transpose(-2, -1),
                    lora_B.unsqueeze(-1),
                    groups=sum(self.enable_lora),
