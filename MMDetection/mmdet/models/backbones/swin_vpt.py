...
class WindowMSA(BaseModule):
    ...
    def forward(self, x, mask=None):
        ...
        if mask is not None:
            ...
            mask = torch.cat(
                (torch.zeros(nW, NUM_PROMPTS, _W, device=attn.device), mask), dim=1
            )
            mask = torch.cat(
                (
                    torch.zeros(nW, _H + NUM_PROMPTS, NUM_PROMPTS, device=attn.device),
                    mask,
                ),
                dim=-1,
            )
...
class ShiftWindowMSA(BaseModule):
    ...
    def __init__(
        ...
        # global v_init
        self.vpt_prompts = nn.Parameter(torch.randn(NUM_PROMPTS, embed_dims))
        nn.init.uniform_(self.vpt_prompts.data, -v_init, v_init)
    ...
    def forward(self, query, hw_shape):
        ...
        nr_windows = int(query_windows.shape[0] / B)
        vpt_prompts = self.vpt_prompts.unsqueeze(0)
        vpt_prompts = vpt_prompts.expand(nr_windows * B, -1, -1, -1)
        vpt_prompts = vpt_prompts.reshape((-1, NUM_PROMPTS, C))
        query_windows = torch.cat((vpt_prompts, query_windows), dim=1)
...
class SwinVPT(BaseModule):
    ...
    def __init__(
        ...
        super(SwinVPT, self).__init__(init_cfg=init_cfg)
        ...
        global v_init
        v_init = math.sqrt(
            6.0 / float(3 * reduce(mul, (patch_size,patch_size), 1) + embed_dims)
        )  # noqa
...
if __name__ == "__main__":
    ...
    train_out = test_model(test_inp)
    print("train done.")

    loss = torch.mean(train_out[0])
    loss.backward()
    ...
    print(torch.sum(train_out[0] - test_out[0]))