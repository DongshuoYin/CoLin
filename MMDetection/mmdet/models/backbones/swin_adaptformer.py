...
class SwinBlock(BaseModule):
    ...
    def forward(self, x, hw_shape):
        def _inner_forward(x):
            ...
            x = self.norm2(x)
            x = self.ffn(x)
            x = x + identity + self.adapter(identity)
...
class SwinAdaptFormer(BaseModule):
    """ Swin Transformer
    ...
    def __init__(
        ...
        super(SwinAdaptFormer, self).__init__(init_cfg=init_cfg)
    ...
    def train(self, mode=True):
        ...
        super(SwinAdaptFormer, self).train(mode)
        self._freeze_stages()
