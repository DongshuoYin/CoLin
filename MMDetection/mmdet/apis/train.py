...
from mmcv.runner import (DistSamplerSeedHook, EpochBasedRunner,
                         ...
                         get_dist_info, GradientCumulativeFp16OptimizerHook, build_optimizer)
...
def train_detector(model,
    ...
    if fp16_cfg is not None:
        optimizer_type = cfg.optimizer_config.get('type', 'Fp16OptimizerHook')
        if optimizer_type == 'Fp16OptimizerHook':
            optimizer_config = Fp16OptimizerHook(
                **cfg.optimizer_config, **fp16_cfg, distributed=distributed)
        elif optimizer_type == 'GradientCumulativeFp16OptimizerHook':
            cfg.optimizer_config.pop('type')
            optimizer_config = GradientCumulativeFp16OptimizerHook(
                **cfg.optimizer_config, **fp16_cfg, distributed=distributed)
        else:
            raise TypeError(
                f'optimizer_config type should be Fp16OptimizerHook or '
                f'GradientCumulativeFp16OptimizerHook, but got '
                f'{optimizer_type}.')
