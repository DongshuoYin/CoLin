...
def main():
    ...
    # --------freeze & calculate--------
    nr_bb_params = 0
    nr_trainalbe_params = 0

    logger.info(f"++++++++++++++++++++++++++++++")

    for n, p in model.named_parameters():
        if "backbone.norm" in n:
            p.requires_grad = True

        if "backbone." in n:
            nr_bb_params += p.numel()
            if p.requires_grad:
                logger.info(f"Parameter {n}, Trainable {p.requires_grad}.")
                nr_trainalbe_params += p.numel()

    logger.info(f"++++++++++++++++++++++++++++++")

    logger.info(f"++++++++++++++++++++++++++++++")
    logger.info(f"Number of Backbone Parameters: {nr_bb_params}.")
    fm = (nr_bb_params-nr_trainalbe_params) if nr_bb_params - nr_trainalbe_params > 0 else nr_bb_params
    logger.info(
        f"Number of Trainable Backbone Parameters: {nr_trainalbe_params} ({nr_trainalbe_params/fm * 100.00} % of backbone parameters.)."
    )
    logger.info(f"++++++++++++++++++++++++++++++")
